"""Both mapping steps persist their deployment-record fragments through the cache.
...
accuracy read). Entries are JSON-safe dicts that re-type via ``from_dict``
cleanly, and the steps' existing stdout/artifacts stay byte-identical.
"""
