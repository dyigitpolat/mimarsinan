...
# ---------------------------------------------------------------------------
# [W4.4] cost-record emission MOVED to the terminal Deployment Record step
# (a fail-loud projection of the sealed record). The simulation step's job
# ends at publishing ``sanafe_simulation_results``.
# ---------------------------------------------------------------------------


def test_step_no_longer_writes_cost_record(monkeypatch):
    import os
    ...
    assert report.sample_indices == [0]
    # The legacy emission is gone from this step: the Deployment Record step
    # writes cost_record.json as a projection of the sealed record.
