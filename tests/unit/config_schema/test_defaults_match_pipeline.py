...
class TestConfigKeysInventory:
    ...
    def test_endpoint_floor_steps_knob_is_a_known_config_key(self):
        """Per-cell RUN-total STEP budget for the 5u endpoint floor (read by
        endpoint_recovery via config.get, TUNING_POLICY fallback). Steps,
        never wall seconds — the reproducibility contract."""
        from mimarsinan.config_schema.defaults import CONFIG_KEYS_SET

        assert "endpoint_floor_steps" in CONFIG_KEYS_SET
        assert "endpoint_floor_wall_s" not in CONFIG_KEYS_SET
