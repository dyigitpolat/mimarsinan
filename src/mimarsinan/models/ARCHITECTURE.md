...
- **Encoding boundary:** Every ComputeOp whose output feeds a neural segment emits a `(T,B,D)` spike train via `mimarsinan.spiking.segment_encoding.emit_compute_spike_train`. For LIF-Perceptron wrappers it runs the perceptron in *single-step* mode for `T` cycles on uniform-encoded raw input (matching NF `run_cycle_accurate`). For bare `Conv*`/`Linear`/`Sequential(Conv*, …)` wrappers it runs the module per-cycle through an ephemeral LIF from `BoundaryLifCache` (`activation_scale` from `hybrid_mapping.node_activation_scales`). Structural passthroughs (mean/flatten/add/etc.) emit `None` (rate-only). Downstream `build_segment_input_spike_train` consumes cached trains verbatim and raises on partial caches under cycle-accurate mode.
