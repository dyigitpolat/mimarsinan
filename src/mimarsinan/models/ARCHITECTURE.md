...
| `hybrid_core_flow.py` | `SpikingHybridCoreFlow` | **Primary** deployable simulator: `HybridHardCoreMapping`, per-core latencies, segment boundaries. Compute stages use `chip_simulation.hybrid_execution.resolve_stage_compute_scales` (always-on). **`forward_with_recording()`** for Loihi/SANA-FE parity. Stage loops are still inline (not `run_hybrid_stages`) due to recording/refcount. |
