...
| `vehicles/` | Deployment-native vehicle models: `stream_cnn.py` (StreamCNN — spiking-native stride-2 conv stack, no pooling, activated FC block + bare-Linear readout; streamable by construction, the P4 streamed-lif conv vehicle) and `narrow_conv.py` (NarrowConvNet — strided Conv-BN stem, constant-width 3x3 stride-2 Conv-BN body stages, a collapse conv consuming the whole residual spatial extent, pointwise trunk stages on the 1x1 map, and a pointwise readout (`bare` or `activated`); every post-stem fan-in is bounded by the DECLARED widths (`9*body_channels+1`, `h*w*body_channels+1`, `trunk_width+1`) rather than by the mapper, so widening or deepening never widens a crossbar row, and no reshape sits between hops — the vehicle for narrow-crossbar per-event targets such as the stock ODIN core, whose event-serial twin folds only whole-input weights fed by another hop) |
...
| `nn/` | NN building blocks: custom autograd activations (`LIFActivation`, TTFS nodes, STE input quantizers), composable activation decorators, standalone layers (`TransformedActivation`, `norm_affine_params`), and shared LIF/TTFS cycle kernels, including `nn/activations/lif_serial.py` — the NF's armed per-event slot, which decomposes the charge through the mapper's own `get_effective_weight`, tiles the upstream event train through the mapper's own unfold (`Mapper.serial_slot_unfold`: one whole-input core for a fully-connected hop, one core per output position for a convolution, each with its own membrane) and checks that decomposition against the fused pre-activation every cycle, refusing by name. |
