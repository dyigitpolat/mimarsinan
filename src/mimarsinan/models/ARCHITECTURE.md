...
| `vehicles/` | Deployment-native vehicle models: `stream_cnn.py` (StreamCNN — spiking-native stride-2 conv stack, no pooling, activated FC block + bare-Linear readout; streamable by construction, the P4 streamed-lif conv vehicle) and `narrow_conv.py` (NarrowConvNet — strided stem, constant-width 3x3 stride-2 body stages, a collapse conv consuming the whole residual spatial extent, bare-Linear readout; every post-stem fan-in is bounded by the DECLARED widths (`9*body_channels+1`, `h*w*body_channels+1`, `head_width+1`) rather than by the mapper, so widening or deepening the vehicle never widens a crossbar row — the vehicle for narrow-crossbar targets such as the stock ODIN core) |
