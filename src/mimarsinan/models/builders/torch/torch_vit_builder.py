...
class TorchViTBuilder:
    ...
    def _build_model(self, num_classes: int) -> VisionTransformer:
        ...
        assert isinstance(conv_proj, nn.Conv2d), "default VisionTransformer patchifies with a single Conv2d"
        model.conv_proj = _adapt_patch_projection(conv_proj, c, patch_size)
        return model
        ...
        def _factory():
            ...
            if c != _VIT_IMAGENET_IN_CHANNELS or patch_size != _VIT_DEFAULT_PATCH_SIZE:
                ...
                if pretrained_proj.bias is not None:
                    state_dict["conv_proj.bias"] = pretrained_proj.bias.detach().clone()
