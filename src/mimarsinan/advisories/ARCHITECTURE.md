...
| `rules_config.py` | Pure functions of the resolved `DeploymentPlan`: ADV-CASC-UNSUPPORTED (cascaded ttfs, casc memo), ADV-STREAMED-CONTRACT (streamed lif per-Neural-Segment execution contract, INFO), ADV-NOVENA-CHARGE (L1 Theorem 0), ADV-STRICT-LT-LATTICE (L1 V9, INFO), and the post-pretrain ADV-ENVELOPE-GATE (M4 §1). Decision axes come from the plan SSOT (the no-stray-flag-reads invariant); mode checks go through `chip_simulation/spiking_semantics.py` predicates, never string ladders. |
| `rules_graph.py` | Topology rules of the mapper `ModelRepresentation` + the `GRAPH_RULES` roster: ADV-STAIRCASE-DEPTH (L from `per_perceptron_cascade_depth` + S; sync composition law × lif 1/S law; carries the WS-W post-QAT adjudication — the trained-composition residual band `STAIRCASE_TRAINED_RESIDUAL_BAND_PP` is AQ capacity and twin-referenced post-hoc folds are closed), ADV-FANIN-DEPTH-IMBALANCE (L1 V6; mapper-graph twin of `mapping/latency/depth_balancing.py`). Thresholds are module constants with one-line justifications. |
| `rules_graph_scale.py` | Scale/grid rules: ADV-SCALE-SPREAD and ADV-NORMFREE-CHAIN (M4; runtime q99 stats when provided, else weight-norm proxies stated in the detail), ADV-BIAS-GRID-DOMINANCE (M2; the same `PerceptronTransformer` effective view the WQ quantizes, gated on `resolve_wq_weight_only_grid` — both routes to a max|w|-only grid cure it). Its predicate is factored out as `worst_bias_grid_dominance` / `bias_dominance_ratio_limit` because the advisory alone cannot certify the WQ entry: the ratio GROWS through conversion, so `WeightQuantizationStep.refuse_bias_dominated_grid` re-reads the SAME predicate on the final tensors and refuses, and `QuantizationVerificationStep.assert_grid_retains_levels` reads the artifact twin (did the largest weight actually retain `BIAS_DOMINANCE_LEVEL_FLOOR` levels) off the diagnostics it already computes. |
