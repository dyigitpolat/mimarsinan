...
| `ttfs_sync_genuine_qat` | Default-off synchronized TTFS QAT gate; allows fast/deployed-staircase training without enabling cascaded segment-spike genuine ramps |
| `fast_ladder_freeze_bn` | Default-off shared fast-ladder policy that keeps BatchNorm modules in eval mode during fast QAT/training steps |
