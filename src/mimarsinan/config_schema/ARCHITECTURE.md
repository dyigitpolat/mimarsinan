...
`CONFIG_KEYS_SET` in `defaults.py` lists keys consumed by steps/tuners/simulation (including `enable_nevresim_simulation`, `enable_loihi_simulation`, `sanafe_*`, `cycle_accurate_lif_forward`, `onchip_majority_gate`/`onchip_majority_min_fraction` — the SoftCoreMappingStep on-chip parameter-majority gate — and `capacity_gate`, the E4 placement-capacity gate (SoftCoreMappingStep `_run_capacity_gate` + scheduler `capacity_precheck`)). Extend it when adding new pipeline config.
