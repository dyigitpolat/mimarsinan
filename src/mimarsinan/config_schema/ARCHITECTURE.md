...
| `deployment_derivation.py` | `derive_deployment_parameters` (quantization/pipeline-mode rules — the ONLY derivation implementation; folds the `ConversionPolicy.derive` recipe authoritatively — sim enables, driver, per-mode knobs), `enforce_quantization_assembly_contract` (bits-driven WQ contract, see “Quantization assembly”), `derive_pipeline_runtime_parameters` (rehydrates firing/spike-generation/thresholding fields), and `derive_platform_constraints` (max_axons/max_neurons from the declared core grid; consistent explicit accepted, contradiction rejected; scalar-only documents skipped). |
| `registry/` | The configurability SSOT (see above): `types.py` (`ConfigKeySchema`/`FieldType`/`Category`, plus `promote_when` — mode-aware prominence: an ADVANCED key renders primary while the predicate holds — and `empty_means` — what an absent value resolves to), `relevance.py` (predicate combinators + JSON codec), `groups.py` (the nine `CONCERN_GROUPS`; the wizard workbench sections host whole groups, so the taxonomy IS the placement), `entries_*.py` (the per-key table; pruning is a conversion-side concern, the ConversionPolicy-owned sim enables and the cores-derived max_axons/max_neurons are DERIVED), `build.py` (assembly, defaults injection from `defaults.py`, import-time coverage validation, `serialize_registry`), `parse.py` (document classification, loud unknown-key report). |
