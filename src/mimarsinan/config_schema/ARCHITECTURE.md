...
| `validation.py` | `validate_deployment_config`, `validate_merged_config`, `s_allocation_config_errors` | JSON / merged config validation; rejects deprecated coalescing keys via `mapping.coalescing`. **EW2:** `s_allocation_config_errors(dp, pc)` validates the per-layer-S declaration — `s_allocation ∈ {uniform\|explicit\|budget}`; `explicit` requires a non-empty positive-int `s_allocation_explicit`; `budget` requires `s_allocation_budget` with ≥1 of {`max_energy_proxy`,`max_latency_steps`,`target`}; the non-uniform modes require the `allow_per_layer_s` chip capability (read from the wrapped/flat platform body). Wired into `validate_deployment_config` (so `validate_wizard_state` enforces it). `uniform` (default) is ungated ⇒ byte-identical. |
