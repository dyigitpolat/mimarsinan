...
| `deployment_derivation.py` | The derivation entry points (it re-exports `derivation/`'s surface, so every historical import path still resolves here): `derive_deployment_parameters` (quantization/pipeline-mode rules — the ONLY derivation implementation; delegates the recipe fold to `recipe_fold.py`; `mirror_training_recipe` reflects the training recipe as-is into the tuning recipe, a document-declared `tuning_recipe` alongside it conflicts loudly), `enforce_quantization_assembly_contract` (bits-driven WQ contract, see “Quantization assembly”), `derive_pipeline_runtime_parameters` (GENERIC over the registry: each legality-bearing key takes its `derived_default` when absent and is judged against its `legal_values` set when declared — an illegal value raises for programmatic callers while the wizard surfaces the same rule as a keyed error — and defaults `negative_value_shift` ON: the boundary-lossless policy whose OFF position is the mapper's subsume-forward path, never a silent clamp). |
| `derivation/` | The derivation rules `deployment_derivation.py` composes: `legality.py` (`legal_value_error` / `legal_values_for` / `legality_bearing_keys` — the legal-value-set law's derivation surface, read straight off the registry) and `platform.py` (`derive_platform_constraints`: max_axons/max_neurons from the declared core grid — consistent explicit accepted, contradiction rejected, scalar-only documents skipped — plus the `target_tq` divides `simulation_steps` cross-key rule), and `soma.py` (`fold_soma_axes`: the EARLY soma-axes fold that materializes `firing_granularity`/`membrane_arithmetic` before the recipe fold and the `sim_enables` derivation, plus `enforce_soma_axes_contract`/`soma_contract_error_rows` — the bits-driven membrane contradiction and the per-event cross-key rules, inert at the default point, raised for programmatic callers and served as keyed remediable rows to the resolve channel)). |
...
## Dependencies
- `chip_simulation` — `spiking_semantics` predicates (`require_known_spiking_mode`, `requires_ttfs_firing`, `forces_activation_quantization`, `is_cycle_based`, `is_cascaded_ttfs`), `activation_semantics.is_streamed_lif` (the NF↔SCM exactness sweep's sample-count derivation), and the legal-value-set / derived-default functions (`legal_firing_modes`, `legal_spike_generation_modes`, `legal_thresholding_modes`, `derived_firing_mode`, `derived_spike_generation_mode`) used by derivation, validation, and the registry entries.
