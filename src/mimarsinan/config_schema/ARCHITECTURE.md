...
| `registry/` | The configurability SSOT (see above): `types.py` (`ConfigKeySchema`/`FieldType`/`Category`), `relevance.py` (predicate combinators + JSON codec), `groups.py` (the nine `CONCERN_GROUPS`; the wizard's step flow maps whole groups to steps, so the workload/model split IS the step assignment), `entries_*.py` (the per-key table), `build.py` (assembly, defaults injection from `defaults.py`, import-time coverage validation, `serialize_registry`), `parse.py` (document classification, loud unknown-key report). |
