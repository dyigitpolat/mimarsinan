...
| `deployment_derivation.py` | `derive_deployment_parameters`, `derive_pipeline_runtime_parameters` | Mirrors deployment derivation rules: float weights → vanilla and disables quantization; cycle-accurate LIF/TTFS and `ttfs_quantized` enable `activation_quantization` preconditioning; runtime spiking fields are rehydrated for minimal persisted configs. Called from `runtime.build_flat_pipeline_config` and `DeploymentPipeline`. |
