...
| `lif_adaptation_tuner.py` | `LIFAdaptationTuner`, `LIFBlendActivation`, `_CycleAccurateForward` | Smooth ramp 0→1 via `LIFBlendActivation` (ReLU-like → LIF). KD: teacher = frozen pre-LIF snapshot; loss = α·CE + (1−α)·T²·KL (T=3, α=0.3). When `cycle_accurate_lif_forward`, patches `model.forward` with picklable `_CycleAccurateForward` → `run_cycle_accurate` (kept after step if cycle-accurate stays on). |
