...
| `../orchestration/kd_blend_adaptation_tuner.py` | `KDBlendAdaptationTuner`, `BlendActivation`, `_KDClassificationLoss`, `_InstalledForward`, `CascadeForwardInstall` | **Shared base** for blend-ramp-with-KD adaptation: snapshots a frozen teacher, installs a `BlendActivation` (old→target by rate) on each perceptron's `base_activation`, ramps 0→1 with `α·CE + (1−α)·T²·KL` (T=3, α=0.3). **Unified ramp shape:** the ramp runs in the value domain by default (`_ramp_forward()` → `None` = the golden, non-destructive per-perceptron `BlendActivation` ramp: rate 0 == continuous teacher bit-exact, rate 1 == pointwise on-chip composition); the genuine cross-layer dynamics are installed at finalize via `_finalize_forward()`. `_after_run` wraps `_continue_to_full_rate`/`_set_rate(1.0)` in a `try/finally: _remove_forward()` (symmetric unpatch) then calls `_finalize` (default: `_update_target_activations()` then install `_finalize_forward()`). `_InstalledForward(model, T)` is the picklable `model.forward` base (subclasses override `_run`; a lazily-built executor is dropped on pickling); `CascadeForwardInstall` is the install/remove mixin (`_install_forward` double-patch assert + idempotent `_remove_forward`). Subclasses override `_make_target_activation`, `_blend_old_activation`, `_make_blend`, `_after_make_target`, `_wrap_encoding_input`, `_ramp_forward`, `_finalize_forward`, `_finalize`; `_append_encoding_input_module` is the shared encoding-input wire-op wrap (Identity→module, else Sequential). |
| `lif_adaptation_tuner.py` | `LIFAdaptationTuner`, `LIFBlendActivation`, `_CycleAccurateForward`, `_ChipAlignedNFForward` | `KDBlendAdaptationTuner` subclass: target = `LIFActivation`. **Finalize:** when `cycle_accurate_lif_forward`, `_finalize` sets `lif_active`, rebuilds activations, applies cycle-accurate trains, and installs `_ChipAlignedNFForward` (`chip_aligned_segment_forward`) as the deployed `model.forward`. **Ramp selection:** `_ramp_forward` returns `None` by default (the golden value-domain ramp); it returns the legacy per-frame `_CycleAccurateForward` (`run_cycle_accurate`) only when `cycle_accurate_lif_forward` AND `legacy_lif_blend_ramp` (default **off**) — the legacy per-frame blend leaks off the continuous teacher at rate 0 (`mean_t ReLU(W s_t + b)`). Both `_CycleAccurateForward` and `_ChipAlignedNFForward` subclass `_InstalledForward`. Symmetric unpatch is inherited from the base. Encoding-layer perceptrons get a `ChipInputQuantizer`. `LIFBlendActivation` subclasses `BlendActivation` (keeps `.lif_activation`). |
| `ttfs_cycle_adaptation_tuner.py` | `TTFSCycleAdaptationTuner`, `_SegmentSpikeForward` | `KDBlendAdaptationTuner` subclass for `ttfs_cycle_based`: target = `TTFSActivation`; sets `adaptation_manager.ttfs_active` so the kernel subsumes the clamp/quant/shift decorators. **Both schedules ramp the value-domain blend gradually** (golden LIF pattern, no rate pin): rate 0 == continuous teacher, rate 1 == pointwise on-chip staircase composition. **cascaded** installs the genuine single-spike cascade (`_SegmentSpikeForward` → `TTFSSegmentForward`, an `_InstalledForward` subclass) only at finalize via `_finalize_forward` and keeps it, so the committed metric and every downstream step run the deployed single-spike dynamics (the intra-segment sub-window timing is not per-core decomposable, so it cannot be the ramp). **synchronized** installs no forward (the class forward through the ramped blend *is* the deployed analytical staircase composition) and trains through the wire contract's stage-input grid snap via a `TTFSInputGridQuantizer` STE on every **segment-entry** perceptron (`segment_entry_perceptrons` — the first on-chip core per segment; NOT `is_encoding_layer`, which is inert under offload and the wrong seam under subsume). With the entry snaps, synchronized NF↔contract is per-neuron bit-exact. **Rung-2 KD** (`ttfs_finetune_kd_against_rung2`, default off, synchronized-only): the KD teacher becomes `_Rung2TeacherFlow` — the frozen pre-step snapshot IR-mapped and evaluated through the identity-mapped contract flow (÷T, per-output node scales) — distilling toward the mapping-level wire semantics at the cost of one IR map + a contract-flow eval per KD batch. |
