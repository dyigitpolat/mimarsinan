...
from mimarsinan.tuning.orchestration.blend_ramp import run_teacher_distmatch
from mimarsinan.tuning.orchestration.frontier.endpoint_recovery import (
    run_endpoint_recovery,
)
from mimarsinan.tuning.orchestration.frontier.prefix_ramp import (
    PrefixConversionRamp,
    run_prefix_stage_reaffine,
)
...
class TTFSCycleAdaptationTuner(KDBlendAdaptationTuner):
    ...
    def _prefix_stage_train(self, rate: float) -> None:
        ...
        self._fast_train_rung(
            float(rate),
            grad_clip_norm=TUNING_POLICY.prefix_stage_grad_clip_norm,
            keep_best_probe=lambda: live_model_acc_fp32(self),
            keep_best_interval=TUNING_POLICY.prefix_stage_keepbest_interval,
        )
