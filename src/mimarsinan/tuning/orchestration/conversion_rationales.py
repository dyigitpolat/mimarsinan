...
_STREAMED_LIF_RATIONALE = (
    ...
    "stays OFF; the T-anneal ladder ramps THROUGH the deployed composition "
    "(TAnnealRealizableRamp installs the tuner's own finalize forward, so "
    "probe == train == deploy at every rung and the rung's T is the walk's T) "
    "and the NF-SCM gate holds bitwise at atol=0. A value-domain proxy ramp "
    "here trained the per-perceptron rate-mode staircase (<= 1 spike/cycle) "
    "instead of the event-serial fold: measured +38.4 pp optimistic at entry "
    "and -41.9 pp at exit on the narrowconv artifacts, which is why "
    "tuning_full_transform_probe is armed to report proxy_gap per rung. "
