...
The five marked rows (``bn_freeze`` / ``rate_subsumed_by_lif`` /
``full_quantile_decode`` / ``fast_only_never_controller`` / ``genuine_qat_fidelity``)
are the documented divergences from the generic flow; each carries a ``rationale``
citing the finding so it stays studyable. ``rate_subsumed_by_lif`` records that rate
takes the LIF recipe knobs yet the pipeline drops it from the LIF reconciliation
steps — so it is not an independently deployable family (it collapses at SCM). Every
proven recipe rides the fast ladder — the controller path is the one that collapses
on the deep cascade, so the SSOT never yields it.
"""
...
# five marked rows below are exactly those documented divergences.
_LIF_RECIPE_KNOBS = {
    "lif_blend_fast": True,
...
_RATE_RATIONALE = (
    ...
    "reconciliation STEPS (LIFAdaptationStep.applies_to=='lif', is_lif_style, "
    "runs_cycle_accurate_activation_tuner), so the deployed count-decode model is "
    "never QAT-reconciled and collapses at Soft Core Mapping (T=4: 0.40-0.50 vs lif "
    "0.965). NOT intrinsic to 4-step coding — lif is the byte-identical natural "
    "control; the only faithful fix routes rate through the LIF reconciliation, which "
    "makes it == lif. rate is therefore subsumed by lif, not an independent deployable "
    "family (mmixcore_verify_24cell / mnist_mixer_fix_wave)."
)
...
class ConversionPolicy:
    ...
    def derive(cls, spiking_mode: str, schedule: Any = None) -> ConversionRecipe:
        ...
        if mode == "lif":
            ...
            # Same recipe KNOBS as lif, but a DISTINCT marker: the pipeline gates
            # exclude rate from the LIF reconciliation steps, so the knobs are not
            # load-bearing at deploy. The collapse is the missing reconciliation,
            # not the 4-step coding (see _RATE_RATIONALE).
            knobs, special_case, rationale = (
                _LIF_RECIPE_KNOBS, "rate_subsumed_by_lif", _RATE_RATIONALE,
            )
