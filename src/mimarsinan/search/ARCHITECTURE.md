...
A problem carries its accountant as `evaluation_budget` (`problem_budget()` is
...
[TS2] The DISCRETE grid behind those continuous vectors is the encoding's to
state, and `encoding.grid_vectors(cap=...)` states it: one coordinate per arch
index, per DISTINCT snapped core dimension (raw values between two granularity
multiples decode to the same chip, so enumerating them all would buy one
candidate many times), per integer count, and per option-axis value. It refuses
rather than truncates — a continuous option axis is refused BY NAME, and a
cartesian product above the caller's declared cap is refused naming both
numbers, because a shortened enumeration is not the exhaustive front anybody
asked for. `arch_search.grid_cap` declares that cap (default
`DEFAULT_GRID_CAP`).
...
[TS2] The sampling backends emit the same triple per BATCH, with
`phase="sampling"` after the first (a drawn batch never evolved, and the panel
shows no pill for a phase it does not know). `total_gens` is the plan's own
batch count — the grid's size for an exhaustive run, `pop_size x generations`
for a drawn one — so a viewer is told what it is waiting for.
