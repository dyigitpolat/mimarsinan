...
_DEFAULT_USER_PROMPT = (
    ...
    "small/medium/large core counts and low/high values of every arch "
    "lever `inspect_search_space` lists. Do NOT cluster all proposals "
    "in one region.\n"
...
def _default_system_instructions(objective_names: Sequence[str]) -> str:
    ...
    return (
        ...
        "(the workload's model levers; `inspect_search_space` lists them).\n"
        "  - `target.kind='hw.core'`, `selector='<core_index>.<dim>'` "
        "where `<dim>` ∈ {`max_axons`, `max_neurons`, `count`}.\n\n"
        "Hardware levers are INTEGER RANGES over the DECLARED search space "
        "(`inspect_search_space` lists each lever's min/max/step): "
        "`max_axons` and `max_neurons` must be multiples of 8 inside their "
        "declared bounds, `count` any integer inside its bounds. A value "
        "outside the declaration is refused before it is built.\n\n"
        ...
        "every response carries its `payload` name and `payload_version`):\n"
        "  - `inspect_softcores(candidate_id)` — per-softcore shape, the "
        "shared bank each core reads, and the source layer that emitted it.\n"
        "  - `inspect_layer_breakdown(candidate_id)` — per-layer counts, "
        "keyed on real layer identity (`perceptron_index`).\n"
        "  - `inspect_weight_banks(candidate_id)` — per shared bank, how "
        "many cores read it (the sharing degree weight-stationary "
        "scheduling exploits).\n"
        "  - `inspect_schedule(candidate_id)` — the composed pass structure "
        "(residency-first, capacity fallback).\n"
        "  - `inspect_capabilities(candidate_id)` — every capability bit "
        "the platform declares.\n"
        "  - `inspect_layout_stats(candidate_id)` — full layout census.\n\n"
