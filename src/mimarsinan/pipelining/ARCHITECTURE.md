...
| `pipelines/` | `DeploymentPipeline` — **`get_pipeline_step_specs(config)`** is the single source of truth for step order (LIF vs TTFS activation chains, optional Loihi/SANA-FE after Simulation); it resolves a `DeploymentPlan` (`core/deployment_plan.py`) and branches on the resolved decisions, not raw `config.get`. `DeploymentPipeline._initialize_config`/`_display_config`/`_assemble_steps` likewise read the plan. See `pipelines/deployment_pipeline.py`. |
