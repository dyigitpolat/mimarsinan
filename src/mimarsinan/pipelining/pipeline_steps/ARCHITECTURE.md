...
**Applicability (Vector V5):** each step declares `@classmethod applies_to(plan)` — whether it belongs in the pipeline for a resolved `DeploymentPlan`. The base `PipelineStep.applies_to` returns `True`; conditional steps override with the per-flag/per-policy predicate that gated their former `append` in `deployment_specs` (e.g. `ArchitectureSearchStep` ↔ `plan.search_mode != "fixed"`, `WeightPreloadingStep` ↔ `bool(plan.weight_source)`, activation preconditioning ↔ `plan.requires_clamp_preconditioning` / `plan.requires_activation_quantization_preconditioning`, cycle tuners ↔ `plan.spiking_mode == "lif"` / `plan.is_ttfs_cycle_based`, the `*QuantizationStep`s ↔ `plan.weight_quantization`). The V5 `StepPlan` registry (`core/step_plan.py`) filters an ordered registry by these predicates; backend steps stay in `BACKEND_REGISTRY`.
