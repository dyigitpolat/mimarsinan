...
| `generate_main.py` | `generate_main_function`, `get_config` | Instantiates `main.cpp` with simulation length, spike/firing modes, **`weight_type`**, **`threshold_type`**. Picks the spike generator type — `{Stochastic,Deterministic,FrontLoaded,Uniform}SpikeGenerator` for rate-coded modes; TTFS modes dispatch into `TTFSContinuousExecution` / `TTFSExecution` separately. |
