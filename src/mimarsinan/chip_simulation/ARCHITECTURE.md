...
| `ttfs_executor.py` | `TtfsAnalyticalExecutor`, `run_ttfs_hybrid_contract` | Canonical TTFS segment semantics and shared hybrid contract runner (numpy float64 compute ops). Applies `node_output_shifts` (negative-value shift) to each assembled neural-stage input. Used by `record_ttfs_hcm_reference` and SANA-FE `contract_ttfs_*` fields. |
