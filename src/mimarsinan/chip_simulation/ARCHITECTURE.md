...
| `hybrid_stage_runner.py` | `run_hybrid_stages` | Shared `for stage in mapping.stages` loop with `on_neural` / `on_compute` callbacks; used by **SANA-FE** and **Lava** `run()`. **nevresim** `SimulationRunner._run_hybrid` still uses an inline loop (profiling + TTFS hooks). |
