...
| `ttfs_encoding.py` | `ttfs_spike_time`, `ttfs_single_spike_train`, `ttfs_latched_spike_train`, `ttfs_input_grid_quantize` | SSOT TTFS input spike encoding (`round(S·(1−clamp(x)))`), spike-train builders, and the encode→decode round-trip `ttfs_input_grid_quantize` used by the synchronized contract input quantization. |
