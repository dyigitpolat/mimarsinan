...
class NevresimDriver:
    ...
    def _prepare_simulator(self, max_input_count, simulation_length, latency, spike_generation_mode, firing_mode):
        print(f"spiking mode: {self.spiking_mode}, firing mode: {firing_mode}")
        generate_main_function(
            self.generated_files_path, max_input_count, self.chip.output_size, simulation_length, latency,
            main_cpp_template, get_config(spike_generation_mode, firing_mode, self.weight_type.__name__, self.spiking_mode))
    ...
    def _run_simulator(self, input_loader, simulation_length, latency, max_input_count=None, num_proc=50):
        """Shared implementation: save inputs, compile, run, return raw float list."""
