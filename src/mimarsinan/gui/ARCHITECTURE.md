...
| `templates.py` | `list_templates`, `get_template`, `save_template`, `delete_template`, `name_and_deployment_from_post_body` | CRUD for deployment configuration templates saved as JSON files. Files are a **flat deployment dict** (same shape as `get_run_config`) but saved through the wizard minimal-persistence builder, so derived/system/runtime defaults are rehydrated later instead of persisted. `save_template` sets `experiment_name` on the stored copy to the given template display name (so lists and wizard load show the template label, not the originating run name). POST `/api/templates` accepts `{name, config}` or a flat deployment body. |
| `wizard/config_builder.py` | `build_deployment_config_from_state` | Canonical wizard persistence builder: writes user-facing deployment/platform knobs only (via `config_schema.namespaced_schema` exposure metadata) and leaves defaults/derived runtime fields to `config_schema.runtime` and `DeploymentPipeline`. |
