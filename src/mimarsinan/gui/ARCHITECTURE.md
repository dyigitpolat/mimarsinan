...
| `snapshot/` | Pure per-artifact snapshot builders returning `(summary, ResourceDescriptor list)`: model, IR graph, hardware mapping, adaptation, pruning, search, and SANA-FE snapshots, `RESOURCE_KIND_*` constants, disk-based snapshot rebuild for legacy runs, and the best-effort console `[TAG]` parser (`console_events.py`) that backfills events for runs recorded before `events.jsonl`. The pruning summary (keyed off the registry's `PRUNING_ADAPTATION_STEP` constant, never a string literal) carries per-layer pre→post neuron/axon dims, achieved sparsity vs the plumbed `pruning_fraction`, a mask-map resource (keep-mask outer product through the same `HeatmapSource` seam as the weight heatmap), and a structured `skipped` `{layer, reason}` list — every skip path (missing masks, length mismatch, bare-Linear fallback, missing model) is a visible tab diagnostic, never a silently empty tab. |
