...
| `wizard/schema.py` | `get_wizard_nas_schema`, `get_pipeline_step_names_for_config` | NAS field defaults aligned with pipeline; `GET /api/wizard/schema` returns `defaults` for fresh wizard sessions. `POST /api/run?validate=1` validates without spawning. |
