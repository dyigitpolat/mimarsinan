...
- **Internal**: `mapping.layout.segmentation` (`partition_ir_graph`, `NeuralSegment`), `mapping.platform.coalescing` (`coalescing_fragment_count`), `mapping.platform.platform_constraints` (`resolve_platform_mapping_params`), `mapping.platform.mapping_structure` (`ChipCapabilities` — SSOT for the `allow_scheduling` permission bit).
