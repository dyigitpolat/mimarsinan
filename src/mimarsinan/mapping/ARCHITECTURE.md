...
6. When `core_types` (typed hardware) is provided, `estimate_passes_for_layout`
   ...
   When a single softcore's coalescing/splitting fragments exceed the available
   core count, the fragment expander (via `_make_softcore_fragment_expander`)
   produces fragment-sized sub-softcores and distributes them across sub-passes.
