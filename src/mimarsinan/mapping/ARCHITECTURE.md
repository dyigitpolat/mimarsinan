...
| `pruning/` | IR pruning, liveness semantics, mask/compaction application, boundary policy, graph segmentation; `certificate/` [W1] is the cascade equivalence certificate (bit-exact value parity of pruned+compacted vs seeded-uncompacted on dyadic-grid instances, zero-preserving host-op registry, shared-bank union rule) — library + tests only, nothing on the default pipeline path calls it |
