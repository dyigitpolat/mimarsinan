...
| `neural_segment_packing.py` | `neural_segment_to_soft_core_mapping` | Default hard-core flush: packs each neural segment's cores into soft cores without `map_fc`. Legacy path: `ir_graph_to_soft_core_mapping` when `use_legacy_softcore_flush` is set. |
| `soft_core_mapper.py` | `SoftCoreMapping`, `map_mm`, `map_fc` | Legacy mapper → SoftCores. **`map_fc`** remains for legacy flush and tests; production flush uses `neural_segment_packing` by default. |
...
| `per_source_scales.py` | `compute_per_source_scales` | Traverses mapper graph for `per_input_scales`; uses `scale_broadcast.broadcast_scale_pair` for `AddMapper`. |
