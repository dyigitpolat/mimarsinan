...
| `chip_latency.py` | `ChipLatency` | Assigns per-core `.latency` for simulation scheduling. (1) Backward walk from outputs; skips `is_off_` axons. (2) `_enforce_core_latency_invariant`: consumer latency ≥ source + 1. (3) `_align_shiftable_cores`: bias-only / dead-input chains shifted to match consumer read windows. Raises if `output_sources` empty. |
