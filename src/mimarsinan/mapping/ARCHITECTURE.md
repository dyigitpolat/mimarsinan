...
| `soft_core_mapper.py` | `SoftCoreMapping`, `map_mm`, `map_fc` | Legacy mapper → SoftCores. **`map_fc` is deprecated** (emits `DeprecationWarning`; prefer `LayoutIRMapping` / `IRMapping`). Still used by `ir_graph_to_soft_core_mapping` and some structural mappers until IR-only path is complete. |
