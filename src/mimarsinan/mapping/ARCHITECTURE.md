...
| `schedule_partitioner.py` | `partition_segment_into_passes`, `estimate_passes_for_layout` | Partitions a neural segment's cores into sequential schedule passes. Respects latency ordering (cores at latency L+1 depend only on latency ≤ L). Coalescing groups may be split across passes — the state buffer handles inter-pass data flow. `_compute_core_latencies` saves/restores original `NeuralCore.latency` to avoid mutation. `estimate_passes_for_layout` is a lightweight layout-level variant for the HW config suggester. |
...
3. If a single latency group exceeds available cores, bin-pack its atomic units.
   Coalescing groups may be split across passes — the state buffer handles inter-pass
   data flow for partial-sum fragments.
4. For heterogeneous core configs, the effective budget is reduced by 20% to account
   for per-type scarcity.
