...
| `platform_constraints.py` | `resolve_platform_mapping_params`, `PlatformMappingParams` | Single source for `hardware_bias`, `effective_max_axons` (`max_axons - 1` when legacy bias axon), and coalescing flag; used by SCM, wizard API, and snapshot builders. |
| `core_geometry.py` | `used_axons`, `used_neurons` | Shared axon/neuron occupancy helpers for HCM, Lava, and SANA-FE. |
| `chip_quantize.py` | `quantize_ir_graph`, `scale_hardware_bias_with_threshold` | IR weight/bias quantization shared by SCM and verification steps. |
