...
| `ir_pruning.py` | `prune_ir_graph`, `get_initial_pruning_masks_from_model`, `get_neural_segments`, `compute_segment_io_exemption` | Eliminates zeroed/pruned rows and columns from the IR; uses model pruning maps when available (else zero-threshold); When model masks are provided, `propagate_when_using_model_masks` (default False) controls whether to run the propagative fixpoint: if False, only the mask-derived rows/cols are pruned (after segment I/O exemption), improving model/flow equivalence; if True, propagation runs and can prune additional structure (maximizes compaction but may reduce accuracy). When no initial masks are given, propagative pruning is always used (delegated to `pruning_propagation`). When neural core count differs from perceptron count (tiled IR), model masks are used only when cores/banks have `perceptron_index` set (sliced by `perceptron_output_slice`/`perceptron_input_slice`); otherwise returns empty masks so only zero-threshold pruning is used. Output nodes use only zero-threshold. For bank-backed cores, Phase 5 sets per-node `pre_pruning_heatmap` and `pruned_row_mask` / `pruned_col_mask` sliced to the node’s effective matrix (by `weight_row_slice` when present) so soft-core compaction and GUI pre/post views see consistent shapes. Segment I/O exemption: per segment, rows fed by segment input (node_id=-2 or external) and columns feeding segment output are never pruned. Pipeline config key `pruning_propagate_with_model_masks` is passed through to `prune_ir_graph` by Soft Core Mapping. |
