...
| `hybrid_build_pool.py` / `hybrid_build_scheduled.py` | `build_hybrid_hard_core_mapping`, `build_identity_hybrid_mapping`, `_build_single_pool`, `_build_scheduled` | IRGraph → hybrid program compiler; both packed paths partition via `layout.segmentation.partition_ir_graph` (single segmentation source). The packing permissions (coalesce/split/schedule) are governed solely by a resolved `platform.MappingStrategy` (param `strategy=`); the legacy three-bool back-compat kwargs were removed (V4 strangler-fig cleanup), so the dispatch reads `strategy.allow_*`, never raw flags. When `strategy` is omitted the all-permissions-off strategy is used (no coalesce/split/schedule). Callers with only raw bools wrap them via `MappingStrategy.from_permissions(allow_coalescing=…, allow_neuron_splitting=…, allow_scheduling=…)`. `build_identity_hybrid_mapping(*, ir_graph)` (re-exported via `hybrid_hardcore_mapping.py`) = 1:1 `NeuralCore`→`HardCore`, no pool/pad/reindex/coalesce/split (SCM rung-2 identity gate) |
...
- `HardCoreMapping.merge_softcore_into` records per-softcore provenance on each placement dict: `ir_node_id` (the source IR `NeuralCore.id`), `perceptron_index`, `perceptron_output_slice` (the tile's neuron range in the **original** layer), `coalescing_role`, and split metadata (`split_group_id`, `neuron_range_in_original`). Per-neuron NF↔HCM spike-count parity reconstructs each perceptron's neurons by the **JOINT `(ir_core_id, neuron_range)` key**: a perceptron's accumulator cores are concatenated in **original-layer neuron-position order** (`perceptron_output_slice[0]`, ir-id as a deterministic tiebreaker), NOT in raw IR-id order, because under coalescing+output-tiling the IR-id assignment is not monotone in the output slice once IR-graph compaction reorders ids — sorting by id then scrambles ~2% of neurons while the deployed value (wired positionally through `output_sources`) and aggregate spike counts stay bit-exact (GAP-1 fix). Neuron-split fragments of one ir-id are reassembled by `neuron_range_in_original` within the tile. Reassemblers: `pipelining.core.nf_scm_parity._group_record_by_perceptron` (production gate) and `tests/integration/_split_reassembly._reassemble` (fidelity-lock harness). (`psum_role` is still recorded but always `None`: the lossy firing partial-sum decomposition was removed — a wide fan-in fuses into one wider crossbar instead.)
