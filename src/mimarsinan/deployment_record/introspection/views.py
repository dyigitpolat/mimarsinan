...
class CandidateLayoutView:
    ...
    def packed(
        ...
        platform: Mapping[str, Any],
        *,
        layout: Optional[LayoutVerificationStats],
        layout_error: Optional[str] = None,
        host_side_segment_count: Optional[int] = None,
        total_params: Optional[float] = None,
    ) -> "CandidateLayoutView":
        """The view of an ALREADY-packed candidate: the packing is given, not redone.

        A caller that has run the layout itself — the search problem's own
        ``candidate_layout`` seam — hands its census over. Re-deriving it here
        would give the reader a SECOND answer that can drift from the one the
        search scored, which is exactly what the channel exists to prevent. The
        capability object and the core grid are still read from the declaration,
        because those are what the answer was computed under.
        """
        return cls(
    ...
    def from_platform(
        ...
        *,
        ...
        For a caller that holds softcores and a chip but no packing yet. The
        capability declaration is forwarded WHOLE (``layout_kwargs``), so the
        pass structure this view reports is the one the builder will compose.
        A caller that ALREADY packed this candidate uses :meth:`packed` instead,
        so the channel never re-answers a question the producer answered.
        """
        capabilities = ChipCapabilities.from_platform_constraints(platform)
        core_types = core_types_from_platform(platform)
