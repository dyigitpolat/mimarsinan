...
|------|---------|---------|
| `data_provider.py` | `DataProvider`, `ClassificationMode`, `RegressionMode` | Abstract base. Subclasses override four per-split dict methods: `raw_datasets()` (datasets shared by both paths), `torch_transforms()` (raw torchvision transform lists; base class wraps with preprocessing in `_assemble_split` → `_wrap_with_preprocessing`), `ffcv_transforms()` (FFCV op chains `[(op_name, kwargs), ...]`; non-empty opts the provider into FFCV — `enable_ffcv()` is derived), and `ffcv_image_field_kwargs()` (kwargs forwarded to `RGBImageField` at beton-write time, e.g. `max_resolution=224`). `get_validation_batch_size()` matches training batch size capped by validation set size. |
...
| `data_providers/` | Concrete providers | MNIST, MNIST-32, CIFAR-10, CIFAR-100, ECG, ImageNet; each may set `DISPLAY_LABEL` for GUI dropdowns. CIFAR-10/100 and ImageNet opt into FFCV by overriding `ffcv_transforms()` + `ffcv_image_field_kwargs()` with explicit per-split op chains. MNIST / MNIST-32 / ECG inherit the empty default (MNIST: no grayscale-collapse op available in stock FFCV; ECG: 1D signal data). |
