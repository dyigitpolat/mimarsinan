...
| `imagenet_data_provider.py` | `"ImageNet_DataProvider"` | ImageNet ILSVRC2012: symlinks `<datasets_path>/imagenet` → `IMAGENET_ROOT` from `.env` when set; train/val = a **seeded-permutation** 95/5 split of `split="train"` (`_seeded_train_val_indices(n, frac, seed)`) — ImageNet train is class-sorted, so a contiguous `range()` split stranded ~50 disjoint classes in val; the permutation makes both holdouts span all 1000 classes, disjoint, reproducible per seed. Test = official `split="val"`. Both `Subset`s view the SAME full train (`.dataset` stays the full train set for `train_imagenet_fast`). Also exposes `fast_fallback_dataloaders(batch_size, num_workers)` — the non-FFCV torchvision fast-recipe path (many workers + `pin_memory` + `persistent_workers` + `prefetch_factor`; train shuffles & drops-last, val/test sequential) consumed by `training.imagenet_fast_train`. This is NOT a pipeline step and does not alter the framework's torch-DataLoader path. |
