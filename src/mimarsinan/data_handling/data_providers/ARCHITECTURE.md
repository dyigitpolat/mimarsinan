...
| `imagenet_data_provider.py` | `"ImageNet_DataProvider"` | ImageNet ILSVRC2012: symlinks `<datasets_path>/imagenet` → `IMAGENET_ROOT` from `.env` when set. **train** = the FULL official `split="train"` (from_scratch training). **val** (the deployment TUNER GATE) + **test** (the reported number) = seeded, DISJOINT slices of the OFFICIAL `split="val"` via `_seeded_val_test_indices(n_official_val, val_fraction, seed)` (`official_val_fraction` default 0.2 → ~10k gate + 40k test). A train holdout would LEAK — a from_scratch/pretrained model has seen every train image, so a train-sourced gate scores memorized (~train) accuracy; sourcing the gate from the official val keeps it genuinely held out for BOTH regimes, and val/test never overlap. Official val is class-sorted, so the seeded permutation also keeps the gate slice representative. Exposes `full_train_dataset()` / `full_official_val_dataset()` (Subset-unwrapping SSOTs used by `train_imagenet_fast`). Also exposes `fast_fallback_dataloaders(batch_size, num_workers)` — the non-FFCV torchvision fast-recipe path (many workers + `pin_memory` + `persistent_workers` + `prefetch_factor`; train shuffles & drops-last, val/test sequential) consumed by `training.imagenet_fast_train`. This is NOT a pipeline step and does not alter the framework's torch-DataLoader path. |
