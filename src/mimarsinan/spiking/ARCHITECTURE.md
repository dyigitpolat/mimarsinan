...
| `scale_aware_boundaries.py` | Set per-block `activation_scale` (theta_out, encoding layer pinned) and forward-propagate `input_activation_scale` via the polymorphic mapper walk; `read_boundary_out_scales` is the pure (no-mutation) twin that DELEGATES non-perceptron nodes to `propagate_boundary_scale` (one implementation, both walks — armed buffer gauges, traffic lifts, residual rules; the §11.2 one-writer law); `verify_boundary_currency_coherence` is the install-seam fail-loud certificate. `input_data_scale` is REQUIRED and stamped on the repr so pure re-reads agree with the propagated values. |
