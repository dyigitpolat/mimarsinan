...
| `channel_scale_equalization.py` | Exact cross-layer per-channel scale migration (M4): walks the mapper DAG for feature-adjacent affine pairs A→ReLU→B whose channel axis is consumed only on linear feature axes (through permute/leading-dim/mean-over-non-channel ops), then migrates `s_c = q99_c/geomean` clipped to `[1/r, r]` (`DEFAULT_CLIP_RATIO=4`, the measured WQ-grid guard) as `W_A ← S⁻¹W_A, b_A ← S⁻¹b_A` (norm-affine realization for BN-attached hops), `W_B ← W_B·S`. Function-preserving by ReLU positive homogeneity; weight-shared axes (token-mixer fc2) are honestly left alone — their exact escape is a per-channel theta, owned by scale propagation. |
...
- `mapping` — `pruning/masks.py` lazily imports `mapping.pruning.boundary_policy.compute_perceptron_io_exemption_indices` to exempt model-I/O layers when computing masks from an IR graph; `channel_scale_equalization.py` imports the mapper node types (`PerceptronMapper`, `ComputeOpMapper`, structural/leading-dim mappers, `ComputeAdapter`) to walk the DAG for exactly-migratable pairs.
