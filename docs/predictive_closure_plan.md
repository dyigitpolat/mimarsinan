...
| R5 verification study | **6/6 SEALED, all gates green** (deepcnn resealing on the multi-pass fix) — dyn-energy ±13% incl. the replicated conv (was ~108x); hops zip 0==0 everywhere; gates caught two real forks live (`tiles`, multi-pass utilization) | `study(R5)` |
