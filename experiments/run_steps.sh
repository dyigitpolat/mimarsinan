...
echo "===== GENUINE CASCADE: MORE TRAINING (+G) -> deployed SCM (baseline steps120 ~0.934) ====="
printf "%-14s %-6s %-10s %-12s %-8s\n" cfg exit ttfs_ft deployed_SCM wall_s
for cfg in exp_st300 exp_st600 exp_st600g; do
  t0=$(date +%s)
  ...
  grep -E "NF.SCM cascaded decision agreement: [0-9.]+|NfScmParityError" /tmp/$cfg.log | tail -1
done
echo "===== STEPS SWEEP DONE ====="
