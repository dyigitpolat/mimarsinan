...
echo "===== BEST CASCADED TTFS RECIPE (mmixcore 9-deep, ANN=0.982) ====="
printf "%-18s %-6s %-10s %-12s %-8s\n" cfg exit ttfs_ft deployed_SCM wall_s
for cfg in exp_best_genuine exp_best_proxy exp_best_stack; do
  t0=$(date +%s)
  ...
  grep -E "NF.SCM cascaded decision agreement: [0-9.]+|Traceback|RuntimeError" /tmp/$cfg.log | tail -1
done
echo "===== BEST DONE ====="
