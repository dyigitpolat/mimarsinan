...
- **`"ttfs_cycle_based"`** — Genuine single-spike TTFS (Stanojevic et al., *Nat. Commun.* 2024). Each neuron fires **exactly one** spike whose timing encodes its value (bias-ramp paced); the backends reconstruct the membrane from incoming spike *timings* rather than injecting real values. By the ReLU↔TTFS equivalence this is **numerically identical** to `ttfs_quantized` (decodes to `floor(S·clamp(z,0,1))/S` = `ttfs_quantized_activation`), so the Python/HCM reference reuses the analytical quantized path; the distinct value is (a) **TTFS-Cycle Fine-Tuning** training through the *exact* kernel (`TTFSCycleActivation`, no half-LSB shift) — see §5.6c — and (b) genuine single-spike simulation in the nevresim/SANA-FE backends. **LIF-style pipeline** (opt-in `enable_ttfs_finetuning`, default on): the fine-tuning step **replaces** the Activation Adaptation / Clamp / Shift / Activation Quantization chain (the `TTFSCycleActivation` clamps + quantises internally, exactly as LIF Adaptation replaces that chain), so `activation_quantization` is forced **off**. With the flag off it falls back to the analytical clamp/shift/quant chain like `ttfs_quantized`.
...
The blend ramp runs in the **value domain** (the plain class forward through `LIFBlendActivation`) — the golden, non-destructive ramp: at rate 0 it reproduces the continuous teacher bit-for-bit. (Legacy, default **off**: when **`legacy_lif_blend_ramp`** is set, the ramp instead installs picklable **`_CycleAccurateForward`** — `run_cycle_accurate` applies the blend per spike frame, which leaks off the continuous teacher at rate 0.) When **`cycle_accurate_lif_forward`** is true, `_finalize` rebuilds the activations and installs **`_ChipAlignedNFForward`** (delegating to `mimarsinan.spiking.chip_aligned_nf.chip_aligned_segment_forward`) as the permanent `model.forward` at finalize — encoding-layer perceptrons run once in rate mode and their outputs are uniform-encoded per cycle for the rest of the graph, mirroring the chip simulators' encoding semantics. The base `_after_run` symmetrically unpatches any ramp forward (`try/finally`) before finalize. All downstream pipeline steps (WQ, NormFusion, SCM probes) then validate against the same forward that Nevresim / SANA-FE / Lava run.
...
Uses `TTFSCycleAdaptationTuner` (a `KDBlendAdaptationTuner` subclass, shared with LIF Adaptation) to ramp each perceptron's `base_activation` (ReLU, possibly GELU/LeakyReLU) to **`TTFSActivation`** — the single-spike TTFS kernel (pointwise staircase `ttfs_quantized_activation·θ`, `S` levels), with a straight-through clamped-ReLU gradient. The ramp runs in the value domain (golden, non-destructive: rate 0 == continuous teacher, rate 1 == pointwise staircase composition), with KD recovery α·CE + (1−α)·T²·KL (T=3, α=0.3) against the pre-blend snapshot. The kernel clamps/quantises internally, so the tuner sets `adaptation_manager.ttfs_active` to subsume the clamp/quant/shift decorators and the chain is skipped (`activation_quantization` forced off). The genuine cross-layer dynamics are installed only at **finalize**: the **cascaded** schedule installs `_SegmentSpikeForward` (the deployed single-spike cascade) as `model.forward` and keeps it through the committed metric and every downstream step; the **synchronized** schedule keeps the class-level analytical staircase composition (its deployed dynamics) and trains the wire-contract `q(x)` grid snap via a `TTFSInputGridQuantizer` STE on each segment entry.
