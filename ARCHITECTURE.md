...
Uses `TTFSCycleAdaptationTuner` (a `KDBlendAdaptationTuner` subclass, shared with LIF Adaptation) to ramp each perceptron's `base_activation` (ReLU, possibly GELU/LeakyReLU) to **`TTFSCycleActivation`** — the *exact* single-spike TTFS kernel (`ttfs_quantized_activation·θ`, no half-LSB shift, `S` levels), with a straight-through clamped-ReLU gradient. KD recovery is α·CE + (1−α)·T²·KL (T=3, α=0.3) against the pre-blend snapshot. The exact kernel clamps/quantises internally, so the tuner sets `adaptation_manager.ttfs_active` to subsume the clamp/quant/shift decorators and the chain is skipped entirely (`activation_quantization` forced off). Unlike LIF, no `model.forward` patching is needed — the single-spike value is a one-pass closed form. This gives the genuine cycle-based execution a train target that exactly matches the deployed kernel.
